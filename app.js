...
(() => {
  ...
  const Auth = {
    ...
    getUser() { const u = localStorage.getItem("cogo_user"); return u ? JSON.parse(u) : null; },
    ...
    async redeemPoints() {
      const token = this.getToken();
      const res = await fetch(`${AUTH_URL}/auth/redeem`, {
        method: "POST",
  ...
  function openAuthModal(tab) {
    tab = tab || "login";
    const existing = document.getElementById("cogoAuthModal");
    ...
    if (Auth.isLoggedIn() && user) {
      modal.innerHTML = `
        <div style="background:#fff;border-radius:12px;padding:28px;max-width:420px;width:100%;position:relative;max-height:90vh;overflow-y:auto;">
          ...
          <h2 style="font-family:serif;margin:0 0 4px;">Merhaba, ${user.name} &#x1F44B;</h2>
          <p style="color:#888;margin:0 0 20px;font-size:13px;">${user.email}</p>
          <div style="background:#fdf8f4;border-radius:8px;padding:16px;text-align:center;margin-bottom:20px;">
            <div style="font-size:28px;font-weight:bold;color:#b87333;" id="userPoints">${user.points||0}</div>
            <div style="font-size:12px;color:#888;">Puan</div>
            <div style="font-size:11px;color:#aaa;margin-top:4px;">Her 10&#x20BA; = 1 puan &#x2022; 100 puan = %10 indirim kodu</div>
            <button onclick="window.redeemPoints()" style="margin-top:12px;background:#1f1a17;color:#fff;border:none;padding:8px 20px;border-radius:6px;cursor:pointer;font-size:12px;">&#x1F381; Kod Al (100 puan)</button>
          </div>
          <div id="discountCodesSection"></div>
          <div id="ordersSection">
            <h3 style="font-size:14px;margin:0 0 10px;">Son Siparişlerim</h3>
            <div id="ordersList" style="font-size:13px;color:#888;">Yükleniyor...</div>
          </div>
          <button onclick="window.logoutUser()" style="width:100%;margin-top:20px;background:#f5f5f5;border:none;padding:10px;border-radius:6px;cursor:pointer;color:#666;font-size:13px;">&#x1F6AA; Çıkış Yap</button>
        </div>`;
      setTimeout(async () => {
        ...
        if (data && data.ok) {
          ...
          if (ol) {
            if (data.orders && data.orders.length > 0) {
              ol.innerHTML = data.orders.map(o => `<div style="border:1px solid #eee;border-radius:6px;padding:10px;margin-bottom:8px;"><div style="font-weight:bold;font-size:12px;">${o.product_name||"Seramik Ürün"}</div><div style="color:#888;font-size:11px;">${(o.created_at||"").slice(0,10)} &bull; ${o.amount}&#x20BA;</div></div>`).join("");
            } else { ol.textContent = "Henüz siparişin yok."; }
          }
          const cs = document.getElementById("discountCodesSection");
          if (cs && data.discount_codes && data.discount_codes.length > 0) {
            cs.innerHTML = "<h3 style=\"font-size:14px;margin:0 0 10px;\">&#x1F3AB; İndirim Kodlarım</h3>" +
              data.discount_codes.map(c => `<div style="background:#f0fff0;border:1px dashed #4caf50;border-radius:6px;padding:10px;margin-bottom:8px;display:flex;justify-content:space-between;"><span style="font-weight:bold;letter-spacing:2px;">${c.code}</span><span style="color:#4caf50;">%${c.discount_pct}</span></div>`).join("");
          }
        }
      }, 100);
    } else {
      ...
      modal.innerHTML = `
        <div style="background:#fff;border-radius:12px;padding:28px;max-width:400px;width:100%;position:relative;">
          <button onclick="document.getElementById('cogoAuthModal').remove()" style="position:absolute;top:12px;right:16px;background:none;border:none;font-size:22px;cursor:pointer;">&#x2715;</button>
          ...
          <div style="display:flex;border-bottom:2px solid #eee;margin-bottom:20px;">
            <button onclick="window.switchAuthTab('login')" style="flex:1;padding:10px;background:none;border:none;font-weight:bold;cursor:pointer;color:${isReg?"#aaa":"#b87333"};border-bottom:${isReg?"none":"2px solid #b87333"};margin-bottom:-2px;">Giriş Yap</button>
            <button onclick="window.switchAuthTab('register')" style="flex:1;padding:10px;background:none;border:none;font-weight:bold;cursor:pointer;color:${isReg?"#b87333":"#aaa"};border-bottom:${isReg?"2px solid #b87333":"none"};margin-bottom:-2px;">Üye Ol</button>
          </div>
          ${isReg ? '<input id="authName" placeholder="Adın Soyadın" style="width:100%;box-sizing:border-box;padding:10px;border:1px solid #ddd;border-radius:6px;margin-bottom:10px;font-size:14px;">' : ""}
          <input id="authEmail" type="email" placeholder="E-posta" style="width:100%;box-sizing:border-box;padding:10px;border:1px solid #ddd;border-radius:6px;margin-bottom:10px;font-size:14px;">
          <input id="authPassword" type="password" placeholder="Şifre" style="width:100%;box-sizing:border-box;padding:10px;border:1px solid #ddd;border-radius:6px;margin-bottom:16px;font-size:14px;">
          <div id="authError" style="color:red;font-size:12px;margin-bottom:10px;display:none;"></div>
          <button onclick="window.submitAuth('${tab}')" style="width:100%;background:#1f1a17;color:#fff;border:none;padding:12px;border-radius:8px;cursor:pointer;font-size:15px;font-weight:bold;">
            ${isReg ? "Üye Ol — 50 Puan Kazan &#x1F381;" : "Giriş Yap"}
          </button>
          ${!isReg ? '<p style="text-align:center;font-size:12px;color:#aaa;margin-top:12px;">Hesabın yok mu? <a href="#" onclick="window.switchAuthTab('register');return false;" style="color:#b87333;">Üye ol</a></p>' : ""}
        </div>`;
    }
  ...
  window.switchAuthTab = function(tab) { openAuthModal(tab); };

  window.submitAuth = async function(tab) {
    const email = (document.getElementById("authEmail")||{}).value||"";
    const password = (document.getElementById("authPassword")||{}).value||"";
    const errEl = document.getElementById("authError");
    if (!email.trim() || !password) { if(errEl){errEl.textContent="Lütfen tüm alanları doldurun.";errEl.style.display="block";} return; }
    let result;
    if (tab === "register") {
    ...
    if (result && result.ok && result.token) {
      ...
      updateAuthButton();
      showToast(tab === "register" ? "&#x1F389; Hoş geldin! 50 puan kazandın." : "&#x2713; Giriş yapıldı.");
    } else {
      if(errEl){errEl.textContent=result&&result.error?result.error:"Bir hata oluştu.";errEl.style.display="block";}
    }
  };
  ...
  window.redeemPoints = async function() {
    ...
    if (result && result.ok) {
      ...
      showToast(result&&result.error ? result.error : "Hata oluştu.");
    }
  };
